...
def parse_text_map(str):
	...
	text_file=open('image/text_image2.map', 'r')
	...
	if (str=='flash'):
		command = "grep" + " 0x0c0 " +  parse_file + " | grep -e '\.o$' -e '\.o)$'";
	else:
		command = "grep -i" + " -e 0x0008 " +  parse_file + " | grep -e '\.o)$' -e '\.o$'";
	...
	for line in proc.stdout:
		...
		if num == 3 :
			if (str=='flash'):
				if (items[0].find("0x0c0")==0 ):
					print >> file, items[0] + " " + items[1] + " " + items[2] 
			else:
				if (items[0].find("0x0008")==0):
					print >> file, items[0] + " " + items[1] + " " + items[2] 
		if num == 4 : 
			if (str=='flash'):
				if (items[1].find("0x0c0")==0 and items[0].find(".debug")!=0):
					print >> file, items[1] + " " + items[2] + " " + items[3]
			else:
				if (items[1].find("0x0008")==0 and items[0].find(".debug")!=0):
					print >> file, items[1] + " " + items[2] + " " + items[3]
	file.close()
	return
...
if os.path.exists('image/text_image2.map'):
	obj_list_gen()
