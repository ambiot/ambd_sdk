...
#define VRAM_ADDR VRAMBuffer
	...
	PSRAM_BSS_SECTION __attribute__ ((aligned (64))) u16 VRAMBuffer[VXSIZE_PHYS * VYSIZE_PHYS];
#else
	__attribute__ ((aligned (64))) u16 VRAMBuffer[VXSIZE_PHYS * VYSIZE_PHYS];
#endif
...
static void _InitController(void) {
	lcd_init(XSIZE_PHYS, YSIZE_PHYS, (U32)VRAM_ADDR);
}
...
static void _SetOrg(int xPos, int yPos) {
  /* TBD by customer */
    int offset;
  offset = xPos + yPos*VXSIZE_PHYS;
  LCDC_DMAImageBaseAddrConfig(LCDC, VRAM_ADDR + offset);
}
