...
//
#define RECOMMENDED_MEMORY (1024L * 70)
...
#else
     0x000000FF,0x00330000,0x00333333,0x00660000
    ,0x00663333,0x00666666,0x00993333,0x00CC3333
    ,0x00CC3434,0x00996633,0x00CC6633,0x00996666
    ,0x009A6666,0x00CC6666,0x00FF6666,0x00CC9966
    ,0x00FF9966,0x00000000,0x00999999,0x00CC9999
    ,0x00FF9999,0x00FF9A9A,0x00FFCC99,0x00CCCCCC
    ,0x00FFCBCB,0x00FFCCCC,0x00FFFFFF
#endif
};
...
static const GUI_WIDGET_CREATE_INFO _aMain[] = {
  ...
  { MULTIPAGE_CreateIndirect, NULL,                GUI_ID_MULTIPAGE0, 160,   0, 160, 240 },
};
...
#else
     0x00FFFFFF,0x00000000,0x0070FF70
#endif
};
...
#else
     0x000000FF,0x00FFFFFF,0x00000000,0x00008000
    ,0x00808080,0x00C0C0C0
#endif
};
...
#else
     0x000000FF,0x00008000,0x00000000,0x00C0C0C0
#endif
};
...
*       _CreateSprites
*/
static void _CreateSprites(void) {
  unsigned i;
  //
  // Create the sprites
  //
  ...
  case WM_NOTIFY_PARENT:
    ...
    case GUI_ID_BUTTON0: // Delete
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if (hTVI0) {
          ...
          GUI_MessageBox("No item selected!", "Info", GUI_MESSAGEBOX_CF_MODAL | GUI_MESSAGEBOX_CF_MOVEABLE);
          WM_SetFocus(hTree0);
        }
        break;
      }
    ...
    case GUI_ID_BUTTON2: // Link
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if (hTVI1) {
          ...
          GUI_MessageBox("No item in buffer!", "Info", GUI_MESSAGEBOX_CF_MODAL | GUI_MESSAGEBOX_CF_MOVEABLE);
          WM_SetFocus(hTree0);
        }
        break;
      }
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if (hTVI0) {
          ...
          if (hTVI1) {
            ...
            GUI_MessageBox("Item is already on top!", "Info", GUI_MESSAGEBOX_CF_MODAL | GUI_MESSAGEBOX_CF_MOVEABLE);
            WM_SetFocus(hTree0);
          }
        }
        break;
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if (hTVI0) {
          ...
          if (hTVI1) {
            ...
            GUI_MessageBox("Item is already on bottom!", "Info", GUI_MESSAGEBOX_CF_MODAL | GUI_MESSAGEBOX_CF_MOVEABLE);
            WM_SetFocus(hTree0);
          }
        }
        break;
    ...
    case GUI_ID_BUTTON5: // New node
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        TREEVIEW_InsertItem(hTree0, TREEVIEW_ITEM_TYPE_LEAF, hTVI0, TREEVIEW_INSERT_FIRST_CHILD, "New.A");
        TREEVIEW_InsertItem(hTree0, TREEVIEW_ITEM_TYPE_LEAF, hTVI0, TREEVIEW_INSERT_BELOW,  "New.B");
        TREEVIEW_InsertItem(hTree0, TREEVIEW_ITEM_TYPE_LEAF, hTVI0, TREEVIEW_INSERT_BELOW,  "New.C");
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if (hTVI0) {
          ...
          if (ItemInfo.IsNode == 0) {
            ...
            GUI_MessageBox("Item is not a leaf!", "Info", GUI_MESSAGEBOX_CF_MODAL | GUI_MESSAGEBOX_CF_MOVEABLE);
            WM_SetFocus(hTree0);
          }
        } else {
          GUI_MessageBox("No item selected!", "Info", GUI_MESSAGEBOX_CF_MODAL | GUI_MESSAGEBOX_CF_MOVEABLE);
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if (hTVI0) {
          ...
          if (ItemInfo.IsNode) {
            ...
            GUI_MessageBox("Item is not a node!", "Info", GUI_MESSAGEBOX_CF_MODAL | GUI_MESSAGEBOX_CF_MOVEABLE);
            WM_SetFocus(hTree0);
          }
        } else {
          GUI_MessageBox("No item selected!", "Info", GUI_MESSAGEBOX_CF_MODAL | GUI_MESSAGEBOX_CF_MOVEABLE);
...
static void _cbTreeview(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
  case MSG_SETUSERALPHA:
    Alpha = pMsg->Data.v;
    WM_InvalidateWindow(pMsg->hWin);
    break;
  case WM_PAINT:
#if (GUI_USE_ARGB)
    GUI_SetUserAlpha(&AlphaState, 255 - Alpha);
#else
    GUI_SetUserAlpha(&AlphaState, Alpha);
#endif
...
static void _InitTreeview(WM_HWIN hWin) {
  ...
  TREEVIEW_ITEM_Handle hNode0;
  TREEVIEW_ITEM_Handle hNode1;
  TREEVIEW_ITEM_Handle hNode2;
  TREEVIEW_ITEM_Handle hNode3;
  ...
  TREEVIEW_InsertItem(hWin, TREEVIEW_ITEM_TYPE_LEAF, hItem , TREEVIEW_INSERT_BELOW      , "1.1.1.1.4");
  ...
  TREEVIEW_InsertItem(hWin, TREEVIEW_ITEM_TYPE_LEAF, hItem , TREEVIEW_INSERT_BELOW      , "1.1.1.4");
  ...
  TREEVIEW_InsertItem(hWin, TREEVIEW_ITEM_TYPE_LEAF, hItem , TREEVIEW_INSERT_BELOW      , "1.2.4");
  ...
  TREEVIEW_InsertItem(hWin, TREEVIEW_ITEM_TYPE_LEAF, hItem , TREEVIEW_INSERT_BELOW      , "1.3.4");
  ...
  TREEVIEW_InsertItem(hWin, TREEVIEW_ITEM_TYPE_LEAF, hItem , TREEVIEW_INSERT_BELOW      , "1.4.4");
  ...
  TREEVIEW_SetBkColor(hWin, TREEVIEW_CI_UNSEL, GUI_MAKE_ALPHA(0x80, GUI_WHITE));
  TREEVIEW_SetBkColor(hWin, TREEVIEW_CI_SEL,   GUI_MAKE_ALPHA(0x80, GUI_BLUE));
  WM_SetCallback(hWin, _cbTreeview);
  WM_SetFocus(hWin);
}
...
*       _DrawRect
*/
static void _DrawRect(int x, int y, int w, int h, U32 Color) {
  GUI_SetColor(Color);
...
static void _cbMain(WM_MESSAGE * pMsg) {
  ...
  case MSG_TIMER:
    ...
    Msg.hWinSrc = hDlg;
    Msg.Data.v  = Alpha;
    Msg.MsgId   = MSG_SETUSERALPHA;
    WM_SendMessage(hItem, &Msg);
    WM_InvalidateWindow(pMsg->hWin);
    pMsg->Data.v = Period;
    break;
  case WM_INIT_DIALOG:
    WINDOW_SetBkColor(hDlg, GUI_INVALID_COLOR);
  ...
  case WM_PAINT:
    ...
    GUI_DispStringHCenterAt("Background", 80, 30);
    _DrawRect(35,  60, 50, 50, GUI_RED);
    _DrawRect(55,  80, 50, 50, GUI_GREEN);
    _DrawRect(75, 100, 50, 50, GUI_BLUE);
    break;
  default:
    WM_DefaultProc(pMsg);
  }
...
static void _cbClient(WM_MESSAGE * pMsg) {
  GUI_ALPHA_STATE   AlphaState;
  static U32        UserAlpha;
  const char      * apText[] = {
    "This is a moveable win-",
    "dow with alphablending.",
    "Please try out by drag-",
    "ging the titlebar . . .",
  };
  unsigned i;
  ...
  switch (pMsg->MsgId) {
  case MSG_TIMER:
    UserAlpha = pMsg->Data.v;
    WM_InvalidateWindow(pMsg->hWin);
    break;
  case WM_PAINT:
#if (GUI_USE_ARGB)
    GUI_SetUserAlpha(&AlphaState, 255 - UserAlpha);
    GUI_SetBkColor(GUI_MAKE_ALPHA((255 - 0xB0), GUI_YELLOW));
#else
    GUI_SetUserAlpha(&AlphaState, UserAlpha);
    GUI_SetBkColor(GUI_MAKE_ALPHA(0xB0, GUI_YELLOW));
#endif
    GUI_Clear();
    ...
    for (i = 0; i < GUI_COUNTOF(apText); i++) {
      ...
      GUI_DispStringHCenterAt(apText[i], 73, GUI_GetDispPosY());
      GUI_DispNextLine();
    }
    GUI_RestoreUserAlpha(&AlphaState);
    break;
...
static void _cbFrame(WM_MESSAGE * pMsg) {
  ...
  case MSG_TIMER:
    ...
    WM_InvalidateWindow(pMsg->hWin);
    pMsg->Data.v = Alpha;
    pMsg->hWin = WM_GetClientWindow(pMsg->hWin);
...
*       _DoAnimation
*/
static void _DoAnimation(WM_HWIN hWin, WM_HWIN hWinMain) {
  WM_MESSAGE Msg;
  U32        TimeNextTree;
  U32        TimeNextSprite0;
  U32        TimeNextSprite1;
  ...
  Time = GUI_GetTime();
  TimeNextTree    = Time + INTERVALL_TREE;
  TimeNextSprite0 = Time + INTERVALL_SPRITE0;
  TimeNextSprite1 = Time + INTERVALL_SPRITE1;
  TimeNextFrame   = Time + INTERVALL_FRAME;
  while (1) {
    Time = GUI_GetTime();
    if (Time >= TimeNextTree) {
      Msg.hWin    = hWinMain;
      Msg.hWinSrc = 0;
      Msg.MsgId   = MSG_TIMER;
      WM_SendMessage(hWinMain, &Msg);
      do {
        TimeNextTree += Msg.Data.v;
      } while (TimeNextTree < Time);
    }
    if (Time >= TimeNextSprite0) {
      _MoveSprite(&_aSprite[0]);
      do {
        TimeNextSprite0 += INTERVALL_SPRITE0;
      } while (TimeNextSprite0 < Time);
    }
    if (Time >= TimeNextSprite1) {
      _MoveSprite(&_aSprite[1]);
      do {
        TimeNextSprite1 += INTERVALL_SPRITE1;
      } while (TimeNextSprite1 < Time);
    }
    if (Time >= TimeNextFrame) {
      ...
      do {
        TimeNextFrame += Msg.Data.v;
      } while (TimeNextFrame < Time);
    }
    GUI_Delay(10);
  }
}
...
void MainTask(void) {
  ...
  //
  // Enable automatic alpha blending
  //
  GUI_EnableAlpha(1);
  //
  // Create dialog with transparent treeview and multipage
  //
  hWinMain = GUI_CreateDialogBox(_aMain, GUI_COUNTOF(_aMain), _cbMain, 0, 0, 0);
  //
  // Create transparent frame window
  //
  hWin = FRAMEWIN_CreateEx(5, 125, 150, 85, WM_HBKWIN, WM_CF_SHOW | WM_CF_HASTRANS, FRAMEWIN_CF_MOVEABLE, 0, "Alpha blending", _cbClient);
  FRAMEWIN_SetFont(hWin, &GUI_Font20B_ASCII);
  FRAMEWIN_SetBarColor(hWin, 0, GUI_MAKE_ALPHA(0xB0, GUI_BLUE));
  FRAMEWIN_SetBarColor(hWin, 1, GUI_MAKE_ALPHA(0xB0, GUI_BLUE));
  WM_SetHasTrans(WM_GetClientWindow(hWin));
