...
struct AudioHwRender {
	 ...
	 */
	int (*GetPresentationPosition)(const struct AudioHwRender *stream, uint32_t *frames, struct timespec *timestamp);
	 ...
	 */
	ssize_t (*Write)(struct AudioHwRender *stream, const void *buffer, size_t bytes);
