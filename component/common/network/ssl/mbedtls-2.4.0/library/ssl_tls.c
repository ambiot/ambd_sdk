...
int mbedtls_ssl_derive_keys( mbedtls_ssl_context *ssl )
{
    ...
    {
#if defined (HMAC_SHA_decrypt_USE_HW)
...
#if defined (HMAC_SHA_decrypt_USE_HW)
            rtl_crypto_hmac_sha1_init(NULL, 0);
            rtl_crypto_hmac_sha1_update(pseudo_hdr, 13);
            rtl_crypto_hmac_sha1_update(ssl->in_iv, ssl->in_msglen);
            rtl_crypto_hmac_sha1_final(computed_mac);
#else
