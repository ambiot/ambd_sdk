...
BaseResponse *epb_unpack_base_response(const uint8_t *buf, int buf_len)
{
    ...
    BaseResponse *response = (BaseResponse *)os_mem_alloc(RAM_TYPE_DATA_ON, sizeof(BaseResponse));
...
BasePush *epb_unpack_base_push(const uint8_t *buf, int buf_len)
{
    ...
    BasePush *push = (BasePush *)os_mem_alloc(RAM_TYPE_DATA_ON, sizeof(BasePush));
...
int epb_auth_request_pack_size(AuthRequest *request)
{
    ...
    {
        pack_size += epb_length_delimited_pack_size(TAG_AuthRequest_Md5DeviceTypeAndDeviceId,
                                                    request->md5_device_type_and_device_id.len);
    }
...
int epb_pack_auth_request(AuthRequest *request, uint8_t *buf, int buf_len)
{
    ...
    tmp = (uint8_t *)os_mem_alloc(RAM_TYPE_DATA_ON, tmp_len);
    ...
    if (request->has_md5_device_type_and_device_id)
    {
        ret = epb_set_bytes(&epb, TAG_AuthRequest_Md5DeviceTypeAndDeviceId,
                            request->md5_device_type_and_device_id.data, request->md5_device_type_and_device_id.len);
        if (ret < 0) { return ret; }
    }
    ...
    if (request->has_mac_address)
    {
        ret = epb_set_bytes(&epb, TAG_AuthRequest_MacAddress, request->mac_address.data,
                            request->mac_address.len);
        if (ret < 0) { return ret; }
    }
    if (request->has_time_zone)
    {
        ret = epb_set_string(&epb, TAG_AuthRequest_TimeZone, request->time_zone.str,
                             request->time_zone.len);
        if (ret < 0) { return ret; }
    }
    if (request->has_language)
    {
    ...
    if (request->has_device_name)
    {
        ret = epb_set_string(&epb, TAG_AuthRequest_DeviceName, request->device_name.str,
                             request->device_name.len);
        if (ret < 0) { return ret; }
    }
...
AuthResponse *epb_unpack_auth_response(const uint8_t *buf, int buf_len)
{
    ...
    AuthResponse *response = (AuthResponse *)os_mem_alloc(RAM_TYPE_DATA_ON, sizeof(AuthResponse));
    ...
    response->aes_session_key.data = epb_get_bytes(&epb, TAG_AuthResponse_AesSessionKey,
                                                   &response->aes_session_key.len);
...
int epb_init_request_pack_size(InitRequest *request)
{
    ...
    {
        pack_size += epb_length_delimited_pack_size(TAG_InitRequest_RespFieldFilter,
                                                    request->resp_field_filter.len);
    }
...
int epb_pack_init_request(InitRequest *request, uint8_t *buf, int buf_len)
{
    ...
    if (request->has_resp_field_filter)
    {
        ret = epb_set_bytes(&epb, TAG_InitRequest_RespFieldFilter, request->resp_field_filter.data,
                            request->resp_field_filter.len);
        if (ret < 0) { return ret; }
    }
    if (request->has_challenge)
    {
        ret = epb_set_bytes(&epb, TAG_InitRequest_Challenge, request->challenge.data,
...
InitResponse *epb_unpack_init_response(const uint8_t *buf, int buf_len)
{
    ...
    InitResponse *response = (InitResponse *)os_mem_alloc(RAM_TYPE_DATA_ON, sizeof(InitResponse));
    ...
    {
        response->auto_sync_max_duration_second = epb_get_uint32(&epb,
                                                                 TAG_InitResponse_AutoSyncMaxDurationSecond);
        response->has_auto_sync_max_duration_second = true;
    }
    if (epb_has_tag(&epb, TAG_InitResponse_UserNickName))
    {
        response->user_nick_name.str = epb_get_string(&epb, TAG_InitResponse_UserNickName,
                                                      &response->user_nick_name.len);
    ...
    {
        response->time_string.str = epb_get_string(&epb, TAG_InitResponse_TimeString,
                                                   &response->time_string.len);
...
SendDataResponse *epb_unpack_send_data_response(const uint8_t *buf, int buf_len)
{
    ...
    SendDataResponse *response = (SendDataResponse *)os_mem_alloc(RAM_TYPE_DATA_ON,
...
RecvDataPush *epb_unpack_recv_data_push(const uint8_t *buf, int buf_len)
{
    ...
    RecvDataPush *push = (RecvDataPush *)os_mem_alloc(RAM_TYPE_DATA_ON, sizeof(RecvDataPush));
...
SwitchViewPush *epb_unpack_switch_view_push(const uint8_t *buf, int buf_len)
{
    ...
    SwitchViewPush *push = (SwitchViewPush *)os_mem_alloc(RAM_TYPE_DATA_ON, sizeof(SwitchViewPush));
...
SwitchBackgroudPush *epb_unpack_switch_backgroud_push(const uint8_t *buf, int buf_len)
{
    ...
    SwitchBackgroudPush *push = (SwitchBackgroudPush *)os_mem_alloc(RAM_TYPE_DATA_ON,
                                                                    sizeof(SwitchBackgroudPush));
    ...
    push->switch_background_op = (EmSwitchBackgroundOp)epb_get_enum(&epb,
                                                                    TAG_SwitchBackgroudPush_SwitchBackgroundOp);
