...
ret_code_t auth_init(ali_init_t const *p_init, tx_func_t tx_func)
{
    ...
    if(g_auth.device_secret_len == DEVICE_SECRET_LEN){
        if(auth_get_device_secret((uint8_t *)&tmp_secret, (uint8_t *)&len) != 0 ){ //case 1.
        ...
        if(auth_get_device_secret((uint8_t *)&tmp_secret, (uint8_t *)&len) == 0){ //case 4.
...
ret_code_t auth_get_device_secret(uint8_t *p_secret, uint8_t *p_length)
{
    ...
    return os_kv_get(DEVICE_SECRET_STR, p_secret, (int *)p_length);
}
