...
#ifdef EXAMPLE_AZURE_IOT_PROVISIONING_SAS
...
#define SAMPLE_NAME AZIOT_PROVISIONING_SAS_SAMPLE
 ...
 */
static void example_azure_iot_provisioning_sas_thread(void* param)
{
...
{
	if(xTaskCreate(example_azure_iot_provisioning_sas_thread, ((const char*)"example_azure_iot_provisioning_sas_thread"), 8000, NULL, tskIDLE_PRIORITY + 1, NULL) != pdPASS)
		printf("\n\r%s xTaskCreate(example_azure_iot_provisioning_sas_thread) failed", __FUNCTION__);
}
