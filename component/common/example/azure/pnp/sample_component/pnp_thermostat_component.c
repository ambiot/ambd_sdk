    ...
    az_span* out_payload)
{
	...
	IOT_SAMPLE_EXIT_IF_AZ_FAILED(az_json_writer_append_property_name(&jw, command_max_temp_name), log);
	IOT_SAMPLE_EXIT_IF_AZ_FAILED(az_json_writer_append_double(&jw, thermostat_component->maximum_temperature, DOUBLE_DECIMAL_PLACE_DIGITS), log);
	IOT_SAMPLE_EXIT_IF_AZ_FAILED(az_json_writer_append_property_name(&jw, command_min_temp_name), log);
	IOT_SAMPLE_EXIT_IF_AZ_FAILED(az_json_writer_append_double(&jw, thermostat_component->minimum_temperature, DOUBLE_DECIMAL_PLACE_DIGITS), log);
	IOT_SAMPLE_EXIT_IF_AZ_FAILED(az_json_writer_append_property_name(&jw, command_avg_temp_name), log);
	IOT_SAMPLE_EXIT_IF_AZ_FAILED(az_json_writer_append_double(&jw, thermostat_component->average_temperature, DOUBLE_DECIMAL_PLACE_DIGITS), log);
    ...
    az_span* out_response)
{
	...
	struct tm timeinfo = sntp_gen_system_time(timezone);
	timeinfo.tm_year -= 1900;
	timeinfo.tm_mon -= 1;
	timeinfo.tm_isdst = -1; //fix mktime parse wrong time
	...
	build_command_response_payload(thermostat_component, start_time_span, end_time_span, response, out_response);
    ...
    az_span* out_payload)
{
	...
	else
	{
		...
		IOT_SAMPLE_EXIT_IF_AZ_FAILED(az_json_token_get_double(&property_value->token, &parsed_property_value), log);

		// Update variables locally.
		ref_thermostat_component->current_temperature = parsed_property_value;
		if (ref_thermostat_component->current_temperature > ref_thermostat_component->maximum_temperature)
		{
		...
		if (ref_thermostat_component->current_temperature < ref_thermostat_component->minimum_temperature)
		{
		...
		ref_thermostat_component->temperature_summation += ref_thermostat_component->current_temperature;
		ref_thermostat_component->average_temperature = ref_thermostat_component->temperature_summation / ref_thermostat_component->temperature_count;
    ...
    az_iot_status* out_status)
{
	...
	{
		// Invoke command.
		if (invoke_getMaxMinReport(thermostat_component, command_received_payload, payload, out_payload))
		{
